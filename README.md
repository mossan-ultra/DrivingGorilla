...
| :------: | :-------: | :-------: |
| 渋滞 | 渋滞の時間をどう楽しむか、そもそも渋滞を防げないのか | 渋滞中にご当地NFTや置きゴリを探索したり、付近の置きゴリとバトルできる |
| 居眠り運転 | 居眠り運転をどう防ぐか、アラート以外で防げないのか | 相棒ゴリラとインタラクティブな会話により居眠り運転を抑止する |
| マンネリ化した移動体験 | 通勤・通学等の移動のマンネリ化を防げないか、より楽しいものに出来ないか | 安全運転、エコドライブでトークンが得られる。周辺のご当地NFTや置きゴリのための寄り道など |
| 長距離移動 | 長距離移動の疲れを減らせないか長距離運転を無くせないのか | 運転時間、運転距離でもトークンが得られる。見たことのないご当地NFTや置きゴリとの出会い |
...
2. 相棒ゴリラを日本全国の好きなエリアに置きゴリラとして配置する事でステーキングによる報酬トークンが得られます。

### Use for Fun!

1. 全国の相棒ゴリラホルダーと相棒ゴリラが手に入れたトークン量をランキングでリアルタイムに競い合える。
2. あなただけの相棒ゴリラと全国のユーザーが配置した置きゴリラでバトルする事ができる。
3. 日本全国のユーザーの相棒ゴリラとランキングで競い合うことができる
4. ドライブ中に手に入れる特別なご当地NFTを自慢できる。
5. 特別なご当地NFTを持っている人だけへの特典を提供。
...
4. 相棒ゴリラを日本全国の好きなエリアに置きゴリラとして配置する事でステーキングすることができる
5. 自動車でのお出かけに限らず、Mobility全般、ウォーキング、ランニング、サイクリング、ツーリング、お買い物、タクシーのお供としても利用可能

### Entertainment

1. 自分だけの相棒ゴリラを安全運転やエコドライブなどで得られる経験値トークンや、ご当地NFTで成長させる事ができる
2. 相棒ゴリラにはChatGPT、音声合成、音声認識を搭載しており、ドライブ中に音声で会話することができる
3. あなたの発話に対する相棒ゴリラの感情をAIによる感情分析でPositive、Natural、Negativeで表現します（感情表現により相棒ゴリラのアニメーションが変化します）
4. ドライブを通じて日本全国の相棒ゴリラとバトルを楽しむことができる
5. 今回の相棒ゴリラは神社巡りが大好きで、マスクメロンが大好物です。魔王も倒したことがあるようです。話しかけてみて下さい。なぞなそやクイズもできるかも
...
- Technical Requirements
    - ネットワークチェーンは「Astar zKEVM」のテストネットのzKatanaを採用<br>
高速で利便性の高いEthereumのL2ネットワーク且つ、ガス代低減による運用費節減を実現
    - UI/UX向上を目的にweb3Authでの認証可能とし、Account Abstraction(AA)に対応<br>
本アプリケーション内web3トランザクションのガスレスを実現
    - Progressive Web Apps(PWA)化により特定のOSやデバイスに依存せず、<br>
PC、スマートフォン、タブレットなど、どのデバイスでもアプリとしてインストールして楽しめる<br>
    - GoogleアカウントなどでSocialLoginが可能
- What web3 can do
    - 自分だけの相棒ゴリラ(NFTアニメーションアート)が貰える
    - web3Authで認証すれば、いつでも、どこでも、どのデバイスでも自分の相棒ゴリラと再会できる
    - 運転データをオンチェーン上に保存し、データに応じた経験値トークンをmint<br>
mintに関わる運転情報は安全運転、エコドライブ、運転時間、運転距離、給油回数。<br>
スマートコントラクトの機能によって運転データから自動的にトークンが払い出される。
    - 特定のスポットにチェックインすることでご当地装備NFTを入手する事ができる<br>
経験値トークンと装備NFTで相棒ゴリラが強くなる<br>
相棒ゴリラのステータスとランキングはいつでも確認できる
- what AI or Generative AI can do
    - ドライブ中でもChatGPTを搭載した相棒ゴリラと音声対話チャットができるよう<br>
音声認識、音声合成で実現<br>
    - 相棒ゴリラの音声は日本の有名なテレビドラマ（VI◯ANTのド◯ムを意識）の翻訳機を表現
    - 相棒ゴリラとのチャットは短いフレーズでも繋がりのある会話のキャッチボールが可能
    - 相棒ゴリラのチャット応答を感情分析する事によりビジュアル表現に変化を持たせる
- What Google Map can do
    - 日本全国の地図上の好きな場所に相棒ゴリラの分身を置きゴリとして配置可能<br>
置きゴリにはステーキング機能を実装（但し、一定確率で脱獄し消滅）
    - Google Map SDKを利用して自車位置に加えて全国に点在するご当地NFT、置きゴリの位置を<br>
ドライブ中でもタイムリーに確認する事が可能
...
| web3Auth | ソーシャルログインを入り口にウォレットを作成することでユーザーのUXを向上。ユーザーによる秘密鍵管理は不要。必要なのはSNSアカウントのみ | 
| Staking | 単なるtoEernではなく、「無くなるリスクはあるが稼いだトークンを増やせる」を追加することで刺激を追加 | 
